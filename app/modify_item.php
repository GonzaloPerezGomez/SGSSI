...
if (isset($_POST['item_modify_submit'])) {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        file_put_contents($error_file, date('Y-m-d H:i:s') . " - Error CSRF. Error con el token en modify_item. Token:" . htmlspecialchars($_POST['csrf_token']) . " IP: " . htmlspecialchars($ip_usuario) . ". User-Agent: " . htmlspecialchars($user_agent) . ". Peticion: " . htmlspecialchars($peticion) . " \n", FILE_APPEND);
		echo "<script>
